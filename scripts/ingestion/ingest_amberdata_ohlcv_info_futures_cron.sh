...
if [ $? -eq 0 ]; then
    echo "$(date) - Script executed successfully" >> "$LOG_PATH/amberdata_ohlcv_info_ingest.log"
else
    echo "$(date) - Script execution failed" >> "$LOG_PATH/amberdata_ohlcv_info_ingest.log"
fi